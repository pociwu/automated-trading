...
- 盤中查價採 Fugle REST 優先；失敗、逾時或方案流量受限時，自動改查臺灣證券交易所 MIS 基本市況報導（免 Key），不會把參考價誤當成最新成交價。
- 行情定時更新只在目前開啟的行情／下單分頁執行；編輯個人資產時不啟動隱藏分頁更新，避免表單焦點與輸入內容被背景重繪干擾。
...
- 股票期初建檔輸入代號後會由臺灣證券交易所行情帶入股名；持有數量以股為單位（1 張為 1,000 股），成本欄輸入每股買進價格，由系統計算總取得成本。
- 錯誤的股票期初資料可由期初建檔頁直接刪除；系統以沖銷方式扣回該筆股數與成本並保留稽核紀錄。
- 「清空重置」可在尚無正式資產異動時清除全部期初部位、期初／沖銷紀錄與快照，並保留資產帳戶供重新建檔；操作前必須輸入確認文字。
...
Fugle 即時成交價預設快取 10 秒、每日漲跌停價格快取 300 秒，避免觀察清單自動更新超過基本方案每分鐘 60 次的限制；可用 `FUGLE_QUOTE_CACHE_SECONDS` 與 `FUGLE_LIMITS_CACHE_SECONDS` 調整。

`TWSE_MIS_URL` 預設指向證交所基本市況報導，不需要另外申請 Key。此備援會用在觀察清單、盤中限價單查價／漲跌停驗證及個人股票估值；為避免網頁備援延遲造成錯誤模擬成交，立即市價買賣與 matcher 撮合仍只接受 Fugle 行情，因此整套 Compose 仍須設定 `FUGLE_API_KEY`。
...
買單在最新成交價小於等於限價、賣單在最新成交價大於等於限價時，以委託限價整筆成交。委託價格必須介於 Fugle 或證交所 MIS 回傳的當日跌停價與漲停價之間。目前不模擬 bid/ask、委託簿順位、部分成交、滑價、手續費、證交稅、交易時段或零股市場。
